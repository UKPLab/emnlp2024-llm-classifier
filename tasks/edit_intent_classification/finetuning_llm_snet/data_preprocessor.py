class DataPreprocessor:
    def __init__(self) -> None:
        ...
        print('Preprocessing the data ...SNet')
    ...
    def create_input_text_and_label(self, sample):
        ...
        sample["text"] = sample["text_old"] + ' ' + sample["text_new"]
       
        input_old = self.tokenizer.encode_plus(sample["text_old"], max_length=self.max_length, padding="max_length", truncation=True, return_tensors="pt")
        input_new = self.tokenizer.encode_plus(sample["text_new"], max_length=self.max_length, padding="max_length", truncation=True, return_tensors="pt")
        
        sample["input_ids_tuple"] = (input_old["input_ids"], input_new["input_ids"])
        sample["attention_mask_tuple"] = (input_old["attention_mask"], input_new["attention_mask"])
