...
from .modelling_peft import AutoPeftModelForSequenceClassificationSiamese
...
class Evaluater:
    ...
    def merge_model(self, finetuned_model_dir:Path, labels, label2id, id2label, emb_type=None):
        ...
        model =  AutoPeftModelForSequenceClassificationSiamese.from_pretrained(
        ...
        model = LlamaForSequenceClassificationSiamese(model.config, model=model.model, score=model.score, emb_type=emb_type)
    ...
    def evaluate(self, test, model=None, tokenizer=None, model_dir=None, output_dir=None,  do_predict = True, 
        ...
        :param response_key: Response key, None for SNet
        """
        # load the model
