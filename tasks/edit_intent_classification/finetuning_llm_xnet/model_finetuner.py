...
def collate_fn(examples, device=None):
    for example in examples:
        ...
        example['len_old'] = torch.as_tensor(len(example['emb1'][0]))
        example['len_new'] = torch.as_tensor(len(example['emb2'][0]))
    ...
    len_old = torch.stack([example["len_old"] for example in examples])
    len_new = torch.stack([example["len_new"] for example in examples])
    
    if device is not None:
        input_ids = input_ids.to(device)
        attention_masks = attention_masks.to(device)
        labels = labels.to(device)
        len_old = len_old.to(device)
        len_new = len_new.to(device)
...
class ModelFinetuner:
    ...
    def fine_tune(self, 
        ...
        if do_train:
            train_result = trainer.train(ignore_keys_for_eval=['len_old', 'len_new'])
